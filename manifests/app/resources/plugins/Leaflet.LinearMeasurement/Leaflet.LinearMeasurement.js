(function(){
  ...
  L.Control.LinearMeasurement = L.Control.extend({
      ...
      initLayer: function(){
          ...
          var id = 1;
          Object.keys(layerHandler.highlights).map(_key =>{
            if(_key.startsWith('mline_')){
                id += 1;
            }
          })
          this._current = 'mline_'+id;
          layerHandler.highlights[this._current] = this.layer;
          layerHandler.highlights[this._current].addTo(this._map);
      },
...
//
      ...
      getDblClickHandler: function(e){
          ...
          layerHandler.addCustomElementToPanel(this._current, this._current.split('_')[1], 'measureLayer');
          ...
          var fireSelected = function(e){
              if(L.DomUtil.hasClass(e.originalEvent.target, 'close')){
                  me._map.removeLayer(workspace);
                  Object.keys(layerHandler.highlights).map(_layer =>{
                    if(layerHandler.highlights[_layer] == workspace){
                        delete layerHandler.highlights[_layer];
                        layerHandler.removeCustomElementFromPanel(_layer);
                    }
                  });
              } else {
