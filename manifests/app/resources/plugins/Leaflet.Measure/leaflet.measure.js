(function (factory, window) {
    ...
    L.MeasureLable = L.Layer.extend({
        ...
        enableClose: function (currentId) {
            this._closeButton = L.DomUtil.create("span", "close", this._container);
            this._closeButton.setAttribute('data-id', currentId);
            this._closeButton.innerHTML =
                '<svg style="pointer-events:none;" class="icon" viewBox="0 0 40 40"><path stroke="#FF0000" stroke-width="3" d="M 10,10 L 30,30 M 30,10 L 10,30" /></svg>';
    ...
    L.MeasureAction = L.Handler.extend({
        ...
        _finishMeasure: function (event) {
            ...
            delete this._directPath;
            delete this._measurePath;

            var _id = this.currentId.split('_');
            layerHandler.addCustomElementToPanel(this.currentId, parseInt(_id[1]) + 1, _id[0]=='mLine'?'measureLayerLine':'measureLayerArea');
        ...
        _resetDirectPath: function (latlng) {
            if (!this._directPath) {
                ...
                if (this.options.model === "area") {
                    index = 'mArea_0';
                    var keys = Object.keys(this.options.layerHandler);
                    if(keys.length > 0){
                        keys = Object.keys(this.options.layerHandler).filter(key=>{
                            if(key.startsWith('mArea_')){
                                return key;
                            }
                        })
                        if(keys.length > 0){
                            index = 'mArea_' + keys[keys.length-1].split('_')[1];
                        }
                    }
                    
                    this.options.layerHandler[index] = new L.Polygon([latlng], {
                        weight: 2,
                        color: this.options.color,
                        dashArray: [5, 5],
                        fillOpacity: 0,
                    ...
                    this.options.layerHandler[index] = new L.Polyline([latlng], {
                        weight: 2,
                        color: this.options.color,
                        dashArray: [5, 5],
                        interactive: false,
                ...
                this._directPath = this.options.layerHandler[index];
                this._map.addLayer(this.options.layerHandler[index]);
                this._trail.overlays.push(this.options.layerHandler[index]);
                //this._trails[index] = this._trail;
            } else {
            ...
            if (!this._measurePath) {
                ...
                if (this.options.model === "area") {
                    ...
                    if(keys.length > 0){
                        ...
                        if(keys.length > 0){
                            index = 'mArea_' + (parseInt(keys[keys.length-1].split('_')[1]) + 1);
                        }
                    }
                    ...
                    if(keys.length > 0){
                        ...
                        if(keys.length > 0){
                            index = 'mLine_' + (parseInt(keys[keys.length-1].split('_')[1]) + 1);
                        }
                    }
                ...
                this._measurePath = this.options.layerHandler[index];
                this._map.addLayer(this.options.layerHandler[index]);
                this._trail.overlays.push(this.options.layerHandler[index]);
                this._trails[index] = this._trail;
            } else {
        ...
        _addLable: function (latlng, content, className, ended, id) {
            ...
            if (ended) {
                var closeButton = lable.enableClose(id);
                L.DomEvent.on(closeButton, "click", this._clearOverlay, this);
            }
        },
        _clear: function(key){
            if(!key){
                if(!this._directPath || this._directPath != this.options.layerHandler[this.currentId]){
                    return false;
                }
                var keys = Object.keys(this.options.layerHandler);
                if(keys.length > 0){
                    ...
                    if(keys.length > 0){
                        key = 'mArea_' + keys[keys.length-1].split('_')[1];
                    }
                }
            }
            if(this._trail && Object.keys(this._trail).length !== 0){
                var i = 0,
            ...
            if(this._trails[key]){
                var i = 0,
                overlays = this._trails[key].overlays,
                    length;
                for (length = overlays.length; i < length; i++) {
                    this._map.removeLayer(overlays[i]);
                }
            }
            if(this.options.layerHandler[key]){
                this._map.removeLayer(this.options.layerHandler[key]);
                delete this.options.layerHandler[key];
            }
            delete this._trails[key];
            layerHandler.removeCustomElementFromPanel(key);
        },
        _clearOverlay: function (event) {
            /*var i = 0,
                overlays = this._trail.overlays,
                length;
            for (length = overlays.length; i < length; i++) {
